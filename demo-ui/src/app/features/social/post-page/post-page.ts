...
export class PostPage {
  ...
  protected async addMedia(post: Post, selection: MediaSelection): Promise<void> {
    await this.attempt(async () => {
      await this.actions.addMedia(post, selection.ref);
      this.pickingMedia.set(false);
    }, "Couldn't add the media.");
  }

  protected async removeMedia(post: Post): Promise<void> {
    if (!(await this.actions.confirm('Remove media', 'Remove the media from this post?', 'Remove')))
      return;
    await this.attempt(() => this.actions.removeMedia(post), "Couldn't remove the media.");
  }
