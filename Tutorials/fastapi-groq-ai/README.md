# FastAPI Groq AI — portable FastAPI chat API (SQLite/D1 + Groq)
...
- **Cloudflare Workers** can use the native D1 binding (`env.DB`) through the
  same protocol.
- **Groq** is the chat provider, called through its OpenAI-compatible API.
  Each chat records which provider it uses; this project owns `groq` chats
  only — Workers AI lives in [`fastapi-cloudflare-ai`](../fastapi-cloudflare-ai).
...
> providers it owns (`chats.provider`), and advertises its own chats link (`groqChats`) on
...
AI reply from Groq) are wired up. Not yet deployed —
...
│   ├── ai.py                    # AI protocol: Groq adapter
...
| A Groq API key | Required for chats | https://console.groq.com/keys |
...
| `GROQ_BASE_URL` | Yes | Groq's OpenAI-compatible base URL |
| `GROQ_MODEL` | Yes | Model used for Groq chats |
...
Groq is called over HTTPS from the Worker, so `pywrangler dev` needs `GROQ_API_KEY` in `.env` (or `.dev.vars`).
...
### 3. Set secrets
Non-secret settings (`GROQ_BASE_URL`, `GROQ_MODEL`, Clerk) are already in
`wrangler.jsonc` `vars`. Only the Groq key is a secret:
...
| POST | `/api/users/{user_id}/chats` | Create a new chat; body `{"provider": "groq"}` (the default). Title is "New chat" until the first message |
...
Chats are answered by `GROQ_MODEL`.
