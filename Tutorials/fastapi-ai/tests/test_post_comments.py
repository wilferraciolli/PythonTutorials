...
async def test_deleted_comment_keeps_its_replies_and_stops_counting(s):
    ...
    shown = [s.comments.to_comment(MEMBER, access, post_row, c) for c in listed]
...
async def test_comment_edit_author_only_and_non_members_cannot_comment(s):
    ...
    with pytest.raises(ForbiddenError):
        await s.comments.update_comment(OWNER, group_id, row["id"], comment["id"], CommentUpdate(body="x"))
    with pytest.raises(ForbiddenError):
        await s.comment(group_id, row["id"], caller=OUTSIDER)

    _, _, edited = await s.comments.update_comment(
        MEMBER, group_id, row["id"], comment["id"], CommentUpdate(body="Hello")
    )
...
async def test_no_comments_on_a_deleted_post(s):
    ...
    with pytest.raises(ConflictError):
        await s.comments.create_comment(MEMBER, group_id, row["id"], CommentCreate(body="x"))
