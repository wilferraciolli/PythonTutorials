...
Templates return the same response envelope as normal GET endpoints, but the
`_data` payload should follow the **create DTO shape**, not the persisted DTO
shape.

That means templates should not include server-managed fields such as:

- `id`
- `created_date`
- resource-level `links`

Those fields are generated by the server after creation and do not need fake
or null values in a create template.
